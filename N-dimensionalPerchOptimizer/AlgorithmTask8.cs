...
{
    public class AlgorithmTask8 : AlgorithmPerch
    {
        public AlgorithmTask8(double U1, double U2, double x0, double x1)
        {
        ...
        public override void I(Perch perch, bool flag = false)
        {
            ...
            {
                x1.Add(x1[i] + 2 * perch.coords[i]);
                x2.Add(- x1[i] * x1[i] + x2[i] + perch.coords[i] * perch.coords[i]);
            }
        ...
        protected override void PoorFlockSwim() // + ГДЕ-ТО ТУТ ЕСТЬ ОШИБКА
        {
