...
{
    public class AlgorithmTask3 : AlgorithmTit
    {
        ...
        public override void I(Tit tit, bool flag = false)
        {
            ...
            x.Add(Math.Pow(x[0], (tit.coords[0])));
            x.Add((1f + tit.coords[1]) * x[1]);
            x.Add(x[2] + tit.coords[2]);

            double res = 0;
            res = (x[0] * x[0] + x[1] * x[1] + (2 * x[2] * x[2] + x[3] * x[3]) * Math.Exp(x[1] * x[1]))
                *
                Math.Sqrt((50 + tit.coords[0] * tit.coords[0] + (tit.coords[1] * tit.coords[1] + tit.coords[2] * tit.coords[2]) * Math.Exp(tit.coords[0] * tit.coords[0])));//, 0.5f);
            tit.fitness = res;
            if (flag == true)
            {
