...
{
    public class AlgorithmTask9 : AlgorithmPerch
    {
        public AlgorithmTask9(double U1, double U2, double x0, double gamma)
        {
            this.x0 = new List<double>(1) { x0 };   //Список одного элемента
            U = new List<Tuple<double, double>>();
            U.Add(new Tuple<double, double>(U1, U2));
            additionalParams = new List<double>();
            additionalParams.Add(gamma);
        }
        ...
        public override void I(Perch perch, bool flag = false)
        {
            ...
            for (int t = 0; t < N_dim; t++)
                res += 1f/(Math.Pow(additionalParams[0], t)) * perch.coords[t] * perch.coords[t]; // * perch.coords[t] * perch.coords[t];
            res = res / 2;
            res += x[x.Count - 1];
